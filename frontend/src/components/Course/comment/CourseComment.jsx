...
const CourseComments = ({ course }) => {
    ...
    return (
        <div className="mt-8">
            {/* هدر نظرات */}
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-zinc-200 dark:border-[#1a2233]">
                ...
                {comments.length > 0 && (
                    <select
                        ...
                        className="appearance-none bg-gray-900 text-white text-sm font-medium px-4 py-2.5 pr-10 rounded-xl border border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent cursor-pointer hover:bg-gray-800 transition-all duration-200 shadow-md bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%27http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%27%20fill%3D%27none%27%20viewBox%3D%270%200%2020%2020%27%3E%3Cpath%20stroke%3D%27%239ca3af%27%20stroke-linecap%3D%27round%27%20stroke-linejoin%3D%27round%27%20stroke-width%3D%271.5%27%20d%3D%27M6%208l4%204%204-4%27%2F%3E%3C%2Fsvg%3E')] bg-[length:20px] bg-[right_10px_center] bg-no-repeat">
                        <option value="newest"
                        ...
                        {canModerate && <option value="unApproved" className="bg-gray-900 text-white">تایید نشده</option>}
                    </select>
                )
